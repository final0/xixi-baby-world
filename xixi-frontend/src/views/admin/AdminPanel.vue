      ...
      <el-tab-pane label="👥 用户管理" name="users">
        <el-table :data="users" stripe>
          ...
          <el-table-column label="管理员" width="80">
            <template #default="{ row }"><el-tag :type="row.isAdmin ? 'danger' : 'info'">{{ row.isAdmin ? '是' : '否' }}</el-tag></template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="100">
            <template #default="{ row }">
              <el-button size="small" :type="row.status ? 'danger' : 'success'" plain @click="toggleUserStatus(row)">
                {{ row.status ? '禁用' : '启用' }}
              </el-button>
            </template>
          </el-table-column>
      ...
      <el-tab-pane label="⚙️ 系统配置" name="config">
        <el-form label-width="120px" style="max-width:540px">
          ...
          <el-form-item label="测试邮件">
            <div style="display:flex;gap:8px">
              <el-input v-model="testEmailTo" placeholder="收件邮箱" />
              <el-button type="primary" @click="handleTestEmail">发送测试</el-button>
            </div>
          </el-form-item>
...
const configLabels = { smtp_host:'SMTP服务器', smtp_port:'SMTP端口', smtp_username:'发件邮箱', smtp_password:'授权码', smtp_ssl:'启用SSL', ai_api_key:'API Key', ai_base_url:'API地址', ai_model:'模型名称' }

onMounted(async () => {
  const homeRes = await getHomeInfo()
  if (homeRes.data.baby) Object.assign(babyForm.value, homeRes.data.baby)
  const userRes = await listUsers()
  users.value = userRes.data.list
  const mottoRes = await listMottos()
  mottos.value = mottoRes.data
  const cfgRes = await getConfigs()
  configs.value = cfgRes.data
})

async function saveBaby() {
  savingBaby.value = true
  try { await updateBaby(babyForm.value); ElMessage.success('保存成功') }
  finally { savingBaby.value = false }
}
async function toggleUserStatus(row) {
  await updateUserStatus(row.id, row.status ? 0 : 1)
  row.status = row.status ? 0 : 1
  ElMessage.success('状态更新成功')
}
async function handleAddMotto() {
  if (!newMotto.value.content) return ElMessage.warning('请输入内容')
  const res = await addMotto(newMotto.value.content, newMotto.value.author)
  mottos.value.unshift(res.data)
  newMotto.value = { content:'', author:'' }
  ElMessage.success('添加成功')
}
async function handleDeleteMotto(id) {
  await ElMessageBox.confirm('确定删除这条短句吗？', '提示', { type:'warning' })
  await deleteMotto(id)
  mottos.value = mottos.value.filter(m => m.id !== id)
  ElMessage.success('已删除')
}
async function toggleMotto(row) {
  await updateMottoStatus(row.id, row.isActive ? 0 : 1)
  row.isActive = row.isActive ? 0 : 1
}
async function saveConfig(key, value) {
  await updateConfig(key, value)
  ElMessage.success(`${configLabels[key]} 已保存`)
}
async function handleTestEmail() {
  if (!testEmailTo.value) return ElMessage.warning('请输入收件邮箱')
  await testEmail(testEmailTo.value)
