      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-position="top">
        ...
        <el-form-item label="邮箱验证码" prop="emailCode">
          <div class="code-row">
            ...
            <el-button :disabled="countdown > 0" @click="handleSendCode" size="large" plain>{{ countdown > 0 ? `${countdown}s后重发` : '获取验证码' }}</el-button>
          </div>
        </el-form-item>
        <el-form-item label="密码" prop="password">
          <el-input v-model="form.password" type="password" placeholder="8-20位，含字母和数字" prefix-icon="Lock" show-password size="large" />
        </el-form-item>
        <el-button type="primary" size="large" :loading="loading" @click="handleRegister" style="width:100%;margin-top:8px">立即加入 🌸</el-button>
      </el-form>
...
const rules = {
  username: [{ required: true }, { min: 4, max: 20, message: '4-20位字符' }, { pattern: /^[a-zA-Z0-9_]+$/, message: '仅字母数字下划线' }],
  email: [{ required: true, message: '请输入邮箱' }, { type: 'email', message: '邮箱格式不正确' }],
  emailCode: [{ required: true, message: '请输入验证码' }],
  password: [{ required: true }, { min: 8, max: 20, message: '8-20位字符' }, { pattern: /^(?=.*[A-Za-z])(?=.*\d).+$/, message: '须含字母和数字' }],
}
...
async function handleRegister() {
  ...
  loading.value = true
  try { await register(form.value); ElMessage.success('注册成功！欢迎加入汐汐的小窝 🌸'); router.push('/login') }
