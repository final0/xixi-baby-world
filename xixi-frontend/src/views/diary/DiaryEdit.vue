    ...
    <div class="edit-form xixi-card">
      <el-form :model="form" label-position="top">
        <el-form-item label="标题" required>
          <el-input v-model="form.title" placeholder="给今天的日记起个标题..." size="large" />
        </el-form-item>
        <div class="form-row">
          <el-form-item label="日期" required style="flex:1">
            <el-date-picker v-model="form.diaryDate" type="date" value-format="YYYY-MM-DD" style="width:100%" size="large" />
          </el-form-item>
          <el-form-item label="心情" style="flex:1">
            <el-select v-model="form.mood" placeholder="选择心情" clearable size="large" style="width:100%">
              ...
              <el-option label="😴 疲惴" value="tired" />
            </el-select>
          </el-form-item>
        </div>
...
onMounted(async () => {
  ...
  quillInstance = new Quill(editorRef.value, {
    theme: 'snow',
    placeholder: '记录汐汐的美好时光...',
    modules: {
      toolbar: [[{ header: [2, 3, false] }], ['bold', 'italic', 'underline'], [{ color: [] }], [{ list: 'ordered' }, { list: 'bullet' }], ['blockquote'], ['clean']]
    }
  })
  if (isEdit.value) {
    const res = await getDiary(route.params.id)
    const d = res.data
    form.value = { title: d.title, diaryDate: d.diaryDate, mood: d.mood || '' }
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateDiary(route.params.id, payload)
