    ...
    <div class="section" v-if="info.featuredPhotos?.length">
      ...
      <div class="photo-grid">
        <div v-for="photo in info.featuredPhotos" :key="photo.id" class="photo-item">
          <el-image :src="photo.thumbnailUrl || photo.url" fit="cover" :preview-src-list="[photo.url]" lazy />
...
.motto-section { background: #fff; border-radius: var(--radius-card); padding: 16px 24px; margin-bottom: 24px; box-shadow: var(--shadow-card); overflow: hidden; min-height: 52px; display: flex; align-items: center; }
