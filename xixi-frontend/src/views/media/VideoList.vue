      ...
      <div v-for="video in videos" :key="video.id" class="video-card xixi-card">
        ...
        <div class="video-info">
          ...
          <div class="video-meta">
            <span>{{ video.takenAt || formatDate(video.createdAt) }}</span>
            <span>{{ video.uploaderNickname }}</span>
          </div>
          <el-button v-if="canDelete(video)" size="small" text type="danger" @click="handleDelete(video.id)">删除</el-button>
        </div>
      </div>
    </div>
    ...
    <el-dialog v-model="uploadVisible" title="上传视频" width="500px">
      <el-form :model="uploadForm" label-width="80px">
        <el-form-item label="视频">
          <el-upload :auto-upload="false" :on-change="f => videoFile = f.raw" accept="video/*" :limit="1">
            <el-button>选择视频文件</el-button>
          </el-upload>
        </el-form-item>
        <el-form-item label="封面图">
          <el-upload :auto-upload="false" :on-change="f => coverFile = f.raw" accept="image/*" :limit="1" list-type="picture-card">
...
async function playVideo(video) {
  ...
  const res = await getPlayUrl(video.id)
...
async function handleUpload() {
  ...
  try {
    ...
    videoFile.value = null; coverFile.value = null
    loadVideos()
